...
static uint64 resolve_group_id_by_name(uint64 schid, const char* name);
static int    csv_contains_id(const char* csv, uint64 wanted);
static void   print_client_link(uint64 schid, anyID clid);
...
const char* ts3plugin_name()
{
    ...
    if (!result) {
        const wchar_t* name = L"Group Finder Plugin"; /* UTF-16 name */
        if (wcharToUtf8(name, &result) == -1) { /* Convert name into UTF-8 encoded result */
            result = "Group Finder Plugin";             /* Conversion failed, fallback here */
        }
    }
    return result;
#else
...
{
    /* If you want to use wchar_t, see ts3plugin_name() on how to use */
    return "This plugin can be used to find users on a server by group.";
}
...
/* helper: return group name for id (or NULL) */
...
static int split_quoted_args(const char* s, char*** outv) {
    ...
    while (*p) {
        ...
        /* parse one token (quoted or bare) */
...
/* resolve group by name (exact, then case-insensitive) -> 0 if not found */
static uint64 resolve_group_id_by_name(uint64 schid, const char* name) {
    if (g_cache.schid != schid || !g_cache.ready) return 0;
    for (size_t i = 0; i < g_cache.count; ++i)
        if (strcmp(g_cache.items[i].name, name) == 0) return g_cache.items[i].id;
    for (size_t i = 0; i < g_cache.count; ++i)
        if (icmp(g_cache.items[i].name, name) == 0) return g_cache.items[i].id;
    return 0;
}
...
/* print clickable link for client */
static void print_client_link(uint64 schid, anyID clid) {
    char* uid  = NULL;
    char* nick = NULL;
    if (ts3Functions.getClientVariableAsString(schid, clid, CLIENT_UNIQUE_IDENTIFIER, &uid)  != ERROR_ok) return;
    if (ts3Functions.getClientVariableAsString(schid, clid, CLIENT_NICKNAME,          &nick) != ERROR_ok) { ts3Functions.freeMemory(uid); return; }

    char line[512];
    snprintf(line, sizeof(line), "[URL=client://%u/%s]%s[/URL]", (unsigned)clid, uid, nick);
    ts3Functions.printMessageToCurrentTab(line);
...
{
    *menuItems = NULL;
    *menuIcon = NULL;    /* All memory allocated in this function will be automatically released by the TeamSpeak client later by calling ts3plugin_freeMemory */
}
...
void ts3plugin_onConnectStatusChangeEvent(uint64 serverConnectionHandlerID, int newStatus, unsigned int errorNumber)
{
    ...
    if (newStatus == STATUS_CONNECTION_ESTABLISHED) { /* connection established and we have client and channels available */
        ...
        cache_reset(serverConnectionHandlerID);
        ts3Functions.requestServerGroupList(serverConnectionHandlerID, NULL);
